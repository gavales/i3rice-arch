...
function getReleaseNotes(params) {
    ...
    var services = {
        ...
        "webservice":"/help/search/releasenotes/doccenter/en/R2018a"
    }
    var errorhandler = function() {
        showAllNotesForReleases(params.releases);
    }
    requestHelpService(params, services, addReleaseNotes, errorhandler);
}
...
function addReleaseNotes(releaseNotes) {
	...
	for (var i = 0; i < groups.length; i++) {
        ...
        var headerHtml = '<span id="' + groupObj.label + '" class="anchor_target"></span><div class="expand ' + expandedClass + '"><h2>' + groupObj.label + '</h2>';
		...
		if (subGroups) {
			for (var j = 0; j < subGroups.length; j++) {
				...
				collapseDiv.append("<h3>" + subGroupObj.label + "</h3>");
                contentDiv.append(collapseDiv);
